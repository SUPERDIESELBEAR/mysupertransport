...
export default function CompanyPayPolicySettings() {
  ...
  const load = useCallback(async () => {
    ...
    const [policyRes, permissionRes] = await Promise.all([
      supabase.from('pay_policies').select('*').eq('is_company_default', true).order('effective_from', { ascending: false, nullsFirst: false }).order('effective_date', { ascending: false }),
  ...
  return <Card className="p-4 space-y-4" data-testid="company-pay-policy-settings">
    ...
    <Dialog open={open} onOpenChange={setOpen}><DialogContent className="max-w-3xl"><DialogHeader><DialogTitle>Open a new company rate sheet</DialogTitle><DialogDescription>Every rate carries forward from the current sheet. Change only the fields that should differ.</DialogDescription></DialogHeader><div className="grid gap-4"><div className="grid gap-1.5"><Label>Effective date</Label><DateInput value={effectiveFrom} onChange={setEffectiveFrom} min={today} /><p className="text-xs text-muted-foreground">The date cannot be earlier than today. Earlier work weeks keep their existing sheet.</p></div><div className="grid gap-3 sm:grid-cols-2">{RATE_FIELDS.map(([key, label]) => <div key={key} className="grid gap-1"><Label htmlFor={`policy-${key}`}>{label}</Label><div className="flex items-center gap-2"><Input id={`policy-${key}`} type="number" min="0" max="100" step="0.01" value={rates[key] ?? ''} onChange={e => setRates(v => ({ ...v, [key]: e.target.value }))} /><span className="text-sm text-muted-foreground">%</span></div><p className="text-[11px] text-muted-foreground">Current: {current ? Number(current[key]) : 0}%{Number(rates[key]) === Number(current?.[key]) ? ' · carried forward' : ' · changed'}</p></div>)}</div></div><DialogFooter><Button variant="outline" onClick={() => setOpen(false)}>Cancel</Button><Button disabled={!formValid} onClick={() => setConfirming(true)}>Review new version</Button></DialogFooter></DialogContent></Dialog>
