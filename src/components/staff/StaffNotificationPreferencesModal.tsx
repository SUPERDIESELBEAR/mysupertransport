...
interface EventTypeDef {
  ...
  iconBg: string;
  iconColor: string;
  roles: AppRole[]; // which roles see this event type
}
...
const EVENT_TYPES: EventTypeDef[] = [
  {
    ...
    description: 'When an assigned operator uploads requested documents',
    icon: Paperclip,
    iconBg: 'bg-muted',
  ...
  },
  {
    ...
    description: 'When an operator completes an onboarding step',
    icon: Target,
    iconBg: 'bg-gold/15',
    iconColor: 'text-gold',
    roles: ['onboarding_staff', 'management'],
  },
  {
    ...
    description: 'When a new driver application is submitted',
    icon: UserCheck,
    iconBg: 'bg-blue-100',
  ...
  },
  {
    type: 'truck_down',
    label: 'Truck Down',
    description: 'When an operator reports a truck down',
    icon: AlertTriangle,
  ...
  },
  {
    ...
    label: 'Dispatch Status Change',
    description: "When an operator's dispatch status changes",
    icon: Truck,
    iconBg: 'bg-muted',
    iconColor: 'text-muted-foreground',
    roles: ['dispatcher', 'management'],
  },
  {
    type: 'new_message',
    label: 'New Message',
    description: 'When you receive a new message from an operator',
    icon: MessageCircle,
    iconBg: 'bg-blue-100',
...
export default function StaffNotificationPreferencesModal({ open, onClose }: Props) {
  ...
  const fetchPrefs = async () => {
    ...
    (data ?? []).forEach(row => {
      if (map[row.event_type] !== undefined) {
        map[row.event_type] = {
