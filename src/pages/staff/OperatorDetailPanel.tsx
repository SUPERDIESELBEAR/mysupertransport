...
import { insertPayload, updatePayload, stampedInsert } from '@/integrations/supabase/helpers';
...
export default function OperatorDetailPanel({ operatorId, onBack, onMessageOperator, onUnsavedChangesChange, onOpenAppReview, expiryOverride, scrollToInspectionBinder, scrollToStageKey, backLabel }: OperatorDetailPanelProps) {
  ...
  // Departing — "may be leaving". Staff-only, reversible, never a termination.
  const [departing, setDeparting] = useState<DepartingFields>({
    is_departing: false, departing_note: null, departing_expected_date: null, departing_at: null,
  });
  ...
  const handleMarkRenewed = async (field: 'cdl' | 'medcert') => {
    ...
    const [{ error }, actorProfile] = await Promise.all([
      supabase.from('applications').update(updatePayload('applications', { [col]: newDateStr })).eq('id', appId),
      actorId
  ...
  const fetchOperatorDetail = async () => {
    ...
    if (op) {
      ...
      setDeparting({
        is_departing: (op as any).is_departing === true,
        departing_note: (op as any).departing_note ?? null,
        departing_expected_date: (op as any).departing_expected_date ?? null,
        departing_at: (op as any).departing_at ?? null,
      });
  ...
  const handleRequestDoc = async (field: keyof OnboardingStatus, label: string) => {
    ...
    try {
      const { error } = await supabase
        .from('onboarding_status')
        .update(updatePayload('onboarding_status', { [field]: 'requested' }))
        .eq('id', statusId);
      if (error) throw error;
  ...
  return (
    <div className="flex flex-col gap-6 animate-fade-in max-w-4xl w-full">
      ...
      <div className="flex flex-wrap gap-2 empty:hidden" style={{ order: isQuickView ? 0 : 7 }}>
        ...
        <DepartingBadge operator={departing} />
      ...
      {/* Departing — may be leaving. Staff-only, reversible, not a termination. */}
      {isActive && (
        <div style={{ order: isQuickView ? 0 : 46 }}>
          <DepartingControl
            operatorId={operatorId}
            operatorName={operatorName}
            value={departing}
      ...
      {(!isQuickView || onboardingHistoryExpanded) && <div style={{ order: isQuickView ? 24 : 16 }}><div className="space-y-3">
        ...
        {(() => {
          ...
          return (
            <div ref={el => { stageRefs.current['stage1'] = el; }} className={`bg-white border rounded-xl shadow-sm transition-colors ${s1Complete ? 'border-status-complete' : 'border-border'}`}>
              <button onClick={() => toggleStage('stage1')} className={`w-full text-left sticky z-20 bg-white rounded-t-xl ${s1Collapsed ? 'rounded-b-xl' : 'border-b border-border/70 shadow-sm'}`} style={{ top: stickyBarHeight }}>
              ...
              {!s1Collapsed && (
                <div className="rounded-b-xl overflow-hidden px-5 pt-4 pb-5 max-w-4xl space-y-4">
                  {/* MVR */}
                  <div className="space-y-2">
        ...
        <div ref={el => { stageRefs.current['stage2'] = el; }} className={`bg-white border rounded-xl shadow-sm transition-colors ${allDocsComplete ? 'border-status-complete' : 'border-border'}`}>
          <button onClick={() => toggleStage('stage2')} className={`w-full text-left sticky z-20 bg-white rounded-t-xl ${s2Collapsed ? 'rounded-b-xl' : 'border-b border-border/70 shadow-sm'}`} style={{ top: stickyBarHeight }}>
            <div className="flex items-center justify-between gap-3 max-w-4xl px-5 py-3">
              <div className="flex items-center gap-2">
          ...
          {!s2Collapsed && (
          <div className="rounded-b-xl overflow-hidden px-5 pt-4 pb-5 max-w-4xl space-y-3">
            <div className="space-y-1.5">
            ...
            {([
              ...
              const markReceivedHandler = async () => {
                ...
                try {
                  const { error: markErr } = await supabase.from('onboarding_status').update(updatePayload('onboarding_status', { [field]: 'received' })).eq('id', statusId);
        ...
        {(() => {
          ...
          return (
            <div ref={el => { stageRefs.current['stage3'] = el; }} className={`bg-white border rounded-xl shadow-sm transition-colors ${s3Complete ? 'border-status-complete' : 'border-border'}`}>
              <button onClick={() => toggleStage('stage3')} className={`w-full text-left sticky z-20 bg-white rounded-t-xl ${s3Collapsed ? 'rounded-b-xl' : 'border-b border-border/70 shadow-sm'}`} style={{ top: stickyBarHeight }}>
                <div className="flex items-center justify-between gap-3 max-w-4xl px-5 py-3">
                  <div className="flex items-center gap-2">
        ...
        {(() => {
          ...
          return (
            <div ref={el => { stageRefs.current['stage4'] = el; }} className={`border rounded-xl shadow-sm transition-colors ${isNa ? 'bg-muted/40 border-border opacity-60' : s4Complete ? 'bg-white border-status-complete' : 'bg-white border-border'}`}>
              <button onClick={() => toggleStage('stage4')} className={`w-full text-left sticky z-20 bg-white rounded-t-xl ${s4Collapsed ? 'rounded-b-xl' : 'border-b border-border/70 shadow-sm'}`} style={{ top: stickyBarHeight }}>
                <div className="flex items-center justify-between gap-3 max-w-4xl px-5 py-3">
                  <div className="flex items-center gap-2">
        ...
        {(() => {
          ...
          return (
            <div ref={el => { stageRefs.current['stage5'] = el; }} className={`bg-white border rounded-xl shadow-sm transition-colors ${borderCls}`}>
              <button onClick={() => toggleStage('stage5')} className={`w-full text-left sticky z-20 bg-white rounded-t-xl ${s5Collapsed ? 'rounded-b-xl' : 'border-b border-border/70 shadow-sm'}`} style={{ top: stickyBarHeight }}>
                <div className="flex items-center justify-between gap-3 max-w-4xl px-5 py-3">
                  <div className="flex items-center gap-2">
        ...
        {(() => {
          ...
          return (
            <div ref={el => { stageRefs.current['stagePE'] = el; }} className={`bg-white border rounded-xl shadow-sm transition-colors ${spComplete ? 'border-status-complete' : 'border-border'}`}>
              <button onClick={() => toggleStage('stagePE')} className={`w-full text-left sticky z-20 bg-white rounded-t-xl ${spCollapsed ? 'rounded-b-xl' : 'border-b border-border/70 shadow-sm'}`} style={{ top: stickyBarHeight }}>
                <div className="flex items-center justify-between gap-3 max-w-4xl px-5 py-3">
                  <div className="flex items-center gap-2">
        ...
        {(() => {
          ...
          return (
            <div ref={el => { stageRefs.current['stage6'] = el; }} className={`bg-white border rounded-xl shadow-sm transition-colors ${s6Complete ? 'border-status-complete' : 'border-border'}`}>
              <button onClick={() => toggleStage('stage6')} className={`w-full text-left sticky z-20 bg-white rounded-t-xl ${s6Collapsed ? 'rounded-b-xl' : 'border-b border-border/70 shadow-sm'}`} style={{ top: stickyBarHeight }}>
                <div className="flex items-center justify-between gap-3 max-w-4xl px-5 py-3">
                  <div className="flex items-center gap-2">
        ...
        {(() => {
          ...
          return (
            <div ref={el => { stageRefs.current['stage7'] = el; }} className={`bg-white border rounded-xl shadow-sm transition-colors ${s7Complete ? 'border-status-complete' : 'border-border'}`}>
              <button onClick={() => toggleStage('stage7')} className={`w-full text-left sticky z-20 bg-white rounded-t-xl ${s7Collapsed ? 'rounded-b-xl' : 'border-b border-border/70 shadow-sm'}`} style={{ top: stickyBarHeight }}>
                <div className="flex items-center justify-between gap-3 max-w-4xl px-5 py-3">
                  <div className="flex items-center gap-2">
      ...
      <div className="empty:hidden" style={{ order: isQuickView ? 9 : 18 }}>{(() => {
        ...
        return (
          <div ref={el => { stageRefs.current[stageKey] = el; }} className="bg-white border border-border rounded-xl shadow-sm">
            <button
            ...
            {!isCollapsed && (
              <div className="rounded-b-xl overflow-hidden border-t border-border divide-y divide-border/60">
        ...
        <FilePreviewModal
          ...
          onSaved={stage2Preview.appField ? async (newUrl: string) => {
            try {
              ...
              const { error } = await supabase.from('applications').update(updatePayload('applications', { [field]: storagePath })).eq('id', appId);
              if (error) {
