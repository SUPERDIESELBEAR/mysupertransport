 ...
 *
 * 2026-09-17: moved onto the shared reader (`migrationSources`), which reads
 * BOTH `supabase/migrations` and `drizzle/migrations`. Reading one folder made
 * this guard blind to every migration the platform tool writes.
 */
...
function newestDefinition(): { file: string; body: string } {
  ...
  for (const { file, path: full } of migrationSources()) {
    ...
    found = { file, body: sql.slice(start, end === -1 ? undefined : end + 11) };
  }
...
describe('consume_application_resume_token — resolved definition', () => {
  ...
  it('reads both migration folders', () => {
    const files = migrationSources().map((s) => s.file);
    expect(files.some((f) => f.startsWith('drizzle/'))).toBe(true);
  });
...
describe('the three functions this pass must not touch', () => {
  it('get_application_by_draft_token, save_application_draft and submit_application_draft are untouched', () => {
    ...
    // Newest migration is the one that pass added; it must not redefine them.
    const newest = sources[sources.length - 1];
    const sql = readFileSync(newest.path, 'utf8').toLowerCase();
    for (const fn of [
      'get_application_by_draft_token',
      'save_application_draft',
      'submit_application_draft',
    ]) {
      expect(sql.includes(`function public.${fn}`), `${newest.file} must not redefine ${fn}`).toBe(false);
    }
  });
});
