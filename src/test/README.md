...
Fails if any device type the deactivation wizard's return list can offer, any live
`equipment_items.device_type`, or any value the `equipment_items_device_type_check` CHECK
permits is not storable in `public.osas_device_type`, if a
