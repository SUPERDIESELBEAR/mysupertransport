...
1. Re-parse the real AAA Freight PDF — source panel renders inline, $50 added to the load total, Total Load Value $1,050, a `load_charges` row saved with `source 'parsed_rate_confirmation'`, broker field shows "AAA Freight Global Inc. — not in directory", stops read Macon / Attalla / Gadsden Warehousing Inc in title case.
2. **Mixed-charge double-counting check.** Build a three-stop load by hand, enter a stop-off amount on the middle stop, and add a separate unattached charge. Confirm Total Load Value counts each amount exactly once (base + FSC + both charges, no duplication from the stop mirror), and that the saved load has exactly two `load_charges` rows — one with `load_stop_id` set to the middle stop, one null — with `loads.total_load_value` matching the on-screen figure. The two-stop AAA load cannot exercise this path, so this is a separate manual run.
3. **Clearing a stop-off amount.** Clear the middle stop's stop-off field and confirm the corresponding entry is removed outright — no zero-amount orphan and no stale amount left behind — both in the live total before saving and in `load_charges` after saving an already-created load. A unit test covers the clear-to-empty transition alongside the double-counting guard.
