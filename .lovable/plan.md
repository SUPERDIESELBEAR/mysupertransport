...
- No schema changes and no new RLS needed. Verified against the live database:
  - Delete is already restricted at the database layer. `brokers` has four policies: `brokers_mgmt_all` (ALL commands, `has_role(auth.uid(),'management') OR has_role(auth.uid(),'owner')`), plus `brokers_staff_select`, `brokers_staff_update`, and `brokers_staff_insert` for dispatcher/onboarding staff. There is no DELETE policy for dispatcher or onboarding staff, so even though `authenticated` holds the table-level DELETE grant, only management and owner can actually delete a row. The UI gate mirrors the boundary rather than being the boundary.
  - `loads_broker_id_fkey` is `ON DELETE RESTRICT`, so a broker with referencing loads cannot be deleted by anyone, including an owner issuing a direct request. That part is enforced server-side. What is missing is a clear reason: a direct delete surfaces a raw `foreign key constraint` error. The dialog will therefore catch Postgres error code `23503` on delete and show "This broker still has loads referencing it and cannot be deleted. Mark it inactive instead." alongside the zero-load UI gate.
  - `broker_documents` and `broker_factoring_history` cascade on broker delete, which is correct for an orphan cleanup; `document_send_log.broker_id` is `ON DELETE SET NULL`.
