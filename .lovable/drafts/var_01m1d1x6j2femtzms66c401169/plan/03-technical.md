...
- The near-pair loop uses `isSoftNearMatch` instead of `editDistance === 1`. Merge, dismiss, Undo and restore behave exactly as they do now, including the shared dismissal records.

The confusable-serial database guard trigger and the unique index on the canonical serial are unchanged — the hard block on true duplicates stays exactly as strict as today.
