...
- Restrictive-policy ROLLOUT: the LIVE-UPDATING (realtime) batch of 19 done 2026-09-17, migration `0010`, after SHARE LINKS + SMALL SETTINGS (`0009`) and the MONEY BATCH (`0007`) — **157 tables done, 2 company-bearing tables still pending** (`company_members` permanently exempt on top of those); live totals **717 policies in `public`, 157 RESTRICTIVE**, linter 170. The owner's live-update check PASSED on the pilot (Driver Hub open in one window, dispatch status changed in another, hub updated without a refresh), and delivery was measured per table before and after the batch: identical. Found on the way: 8 tables are subscribed in source but NOT in the `supabase_realtime` publication, so those subscriptions deliver nothing today (pre-existing); the pre-check's realtime list wrongly included `loads` and `deductions`. What remains: `contractor_pay_setup` (driver pay data on the driver's own screens — money-shaped pass of its own) and `user_roles`. (record 2026-09-17 2350 UTC — "the LIVE-UPDATING (realtime) batch")
