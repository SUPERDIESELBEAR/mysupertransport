...
- Restrictive-policy ROLLOUT: the twelve newly stamped tables done 2026-09-17 after the ELD/RODS batch — **110 tables done, 50 company-bearing tables still pending** (`company_members` permanently exempt among them); live totals **670 policies in `public`, 110 RESTRICTIVE**, 160 tables now carry `company_id`. The remaining groups are MONEY (invoices, invoice lines and batches, payments, settlements settings, factoring remittances, AR aging, accessorial adjustments, the number configs), LIVE-UPDATING (realtime-subscribed tables, owner's live-update check first), SHARE LINKS (`share_tokens` and the token tables), SMALL SETTINGS, and `user_roles`. `operators` and `passenger_authorizations` are additional realtime exclusions found in source but missing from the pre-check's realtime list. Batch order and the fuel caveat (derive from `fuel_import_batches`, never from the operator — now applied) are in the pre-check. (record 2026-09-17 (later) — "the twelve unstamped tables"; `docs/passes/2026-09-16-1920-restrictive-policy-precheck.md`)
...
- Next tenancy batch: the twelve MONEY tables (`invoices`, `invoice_line_items`, `invoice_batches`, `invoice_number_config`, `payments`, `factoring_remittances`, `ar_aging_snapshots`, `accessorial_adjustments`, `settlement_settings`, `carrier_signature_settings`, `share_tokens`, `unit_number_config`). **These are NOT a decision of the 2026-09-16 disposition** — that disposition's twelve were the fuel/operator/staff tables, stamped and closed 2026-09-17. They are the twelve tables the 2026-09-16 read-enforcement CENSUS found already carrying `company_id` and already testing it on reads (`(company_id = current_company_id()) AND <role test>`); what they still lack is the restrictive `tenant_isolation` policy. (record 2026-09-16 — "the owner's disposition decision, and the live read-enforcement census", section (c); correction 2026-09-17 2050, section (a))
