...
- Restrictive-policy ROLLOUT: the SHARE LINKS (5) and SMALL SETTINGS (2) batch done 2026-09-17, migration `0009`, after the MONEY BATCH (21) in `0007` — **138 tables done, 21 company-bearing tables still pending** (`company_members` permanently exempt on top of those); live totals **698 policies in `public`, 138 RESTRICTIVE**, linter 170. Every signed-out share-link path runs through a SECURITY DEFINER function or a service_role edge function, so the `TO authenticated` rule cannot reach an anonymous visitor — verified live, before and after, on every link kind that exists. What remains: `contractor_pay_setup` (driver pay data on the driver's own screens — money-shaped pass of its own), LIVE-UPDATING (realtime-subscribed tables, owner's live-update check first), and `user_roles`. `operators` and `passenger_authorizations` are additional realtime exclusions found in source but missing from the pre-check's realtime list. Batch order and the fuel caveat (derive from `fuel_import_batches`, never from the operator — applied) are in the pre-check. (record 2026-09-17 2330 UTC — "share links and small settings"; `docs/passes/2026-09-16-1920-restrictive-policy-precheck.md`)
