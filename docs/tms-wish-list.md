...
- **DEMO CARRIER STAGE 3 of 6: COMPLETE 2026-09-23 1745 UTC (3a–3e built; see the stage 3 entry at the end). Original design note follows.** Owner decision 2026-09-23: applications and the PEI family become per-carrier, superseding the 2026-09-13 global decision. TRIGGER for building: **how an anonymous applicant's record gets a carrier.** Recommended, not chosen: a **per-carrier apply link**, with the invite path falling out of it and a sole-carrier fallback behind it — the only option that also fixes the fact that `useCompanyIdentity` cannot read `carrier_profile` as `anon` and therefore prints the hard-coded SUPERTRANSPORT name and DOT on **every carrier's applicant disclosures**. The demo alone needs only the invite path. A default-carrier flag would keep `company_id` nullable forever. Recorded live facts: 346 applications (77 drafts), 144 PEI requests, **zero unexpired resume tokens** so no token in the wild can break, `pei_cadence_settings` already per-carrier, and the family's **only** anonymous table privilege is `anon`'s INSERT on `applications`, which the design closes entirely. `profiles` follows and must never get a `company_id`. Five build passes named (3a-3e). (record 2026-09-23 1310 UTC; `docs/passes/2026-09-23-1310-applications-per-carrier-design.md`)
...
- ~~**FIVE CRON-TARGET FUNCTIONS HAVE NO CALLER CHECK AT ALL**~~ — **CLOSED 2026-09-24 14:39 UTC** (docs/passes/2026-09-24-1439-alvys-m1-load-history.md). All five gate on `_shared/cronAuth.ts` (x-cron-secret = CRON_SECRET, or service role); `notify-pwa-install` also admits a signed-in staff session. Proven live: publishable key refused by all five (403 ×4, 401 notify-pwa-install); staff call admitted. The first scheduled runs after the change must be read from `net._http_response`. (record 2026-09-21 0105 UTC)

- ~~**DRIVERS CANNOT MOVE A LOAD'S STATUS**~~ — CLOSED 2026-09-24 (Alvys M1 pass 2, migration 0065). A recorded arrival or departure at a stop now moves the load forward automatically, the same way whether the driver or dispatch recorded it (P62, P63); never backward. Drivers still cannot set a status by hand. Report: docs/passes/2026-09-24-2330-stop-times-move-status.md.
- **Broker link — live truck location from Motive** (needs Motive API access; share only while the truck is on that load). Future, per P64. Added 2026-09-24 (Alvys M1 pass 3).
- **Broker link — show the dispatcher how often and when the broker opened it.** Future; the opens are already logged in share_token_access_log. Added 2026-09-24 (Alvys M1 pass 3).
...
- **Remittance posting has never worked — FIXED 2026-09-25 (0068, P70): payments now written with source `factor`; proven posted, invoice paid, load paid (`docs/passes/2026-09-25-1243-invoice-pdf.md`).** Was: `post_invoice_payment_internal` writes `payments.source = 'factoring'`, but `payments_source_check` allows only `factor`, `broker`, `other`. Every matched remittance line fails with 23514 and the whole remittance rolls back. `factoring_remittances` holds 0 rows, so nothing real was affected. One-word fix; belongs to the payout-PDF upload and matching pass (P67), with an owner yes first. (`docs/passes/2026-09-25-1104-billing-company-checks.md`)
- **Dispatchers cannot read invoice lines — FIXED 2026-09-25 (0068, P71): read-only `invoice_line_items_view_permission` under invoice.view; Leo reads 1 line, UPDATE/DELETE 0 rows.** Was: Through `invoice.view` a dispatcher reads invoices (proof: 2 of 2) but 0 invoice lines; `invoice_line_items` has no permission-based SELECT policy. The Billing queue does not read lines, so issuing works. Needs an owner yes before adding a SELECT policy.
