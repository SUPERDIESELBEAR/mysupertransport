...
- Five migration-reading guards still read `supabase/migrations` only, so a migration written to `drizzle/migrations` is invisible to them: `resume-token-reuse`, `notification-priority`, `settlement-foundation` (the escrow/holdback vocabulary walk), `src/lib/__tests__/settlementRun.test.ts`, and `verbatimVerificationCard.test.tsx`. Each owns a local `readdirSync`. (record 2026-09-17 — "the Drizzle migration folder, explained", the readers table)
