...
- Restrictive-policy ROLLOUT: the MONEY BATCH (21 tables) done 2026-09-17, migration `0007` — **131 tables done, 28 company-bearing tables still pending** (`company_members` permanently exempt on top of those); live totals **691 policies in `public`, 131 RESTRICTIVE**. The remaining groups are LIVE-UPDATING (realtime-subscribed tables, owner's live-update check first), SHARE LINKS (`document_short_links`, `officer_packet_links`, `ica_review_links`, `binder_share_bundles`, `preview_sessions`), SMALL SETTINGS, and `user_roles`. `operators` and `passenger_authorizations` are additional realtime exclusions found in source but missing from the pre-check's realtime list. Batch order and the fuel caveat (derive from `fuel_import_batches`, never from the operator — applied) are in the pre-check. (record 2026-09-17 2200 UTC — "the money batch"; `docs/passes/2026-09-16-1920-restrictive-policy-precheck.md`)
...
- HARNESS GAP: `grant_parity_report()` — the ONE check comparing live policies against live grants — cannot be called by the test harness. Migration 0004's grant landed on `sandbox_exec_<project>`, not on `sandbox_exec`, the role psql connects as; granting it to `authenticated` or PUBLIC is not acceptable because it reads every table's grants. The call is now gated with a loud skip banner. A grant made or revoked out of band would go unseen until this runs as a role that may call it. (record 2026-09-17 2200 UTC — section (e))
