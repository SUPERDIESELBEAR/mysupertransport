...
- Cutover purge procedure: re-read end to end before it runs. (record — "Cutover purge procedure — authoritative, execute on cutover day", `docs/tms-build-status.md`, and the warning at "The cutover purge procedure was written 2026-09-03. Every pass after it did the right thing individually". NOTE: the record's procedure is numbered by step and its live step count was NOT verified in this pass, so the "13-step" label from the prompt is not asserted here.)
