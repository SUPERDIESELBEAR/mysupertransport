...
- **DRIVERS CANNOT MOVE A LOAD'S STATUS** (found 2026-09-24, Alvys M1 pass 1). `enforce_loads_operator_update` allows a driver only accept/decline/reefer-ack fields, and `update_load_status` admits dispatcher/management/owner only. So the history's `driver_app` source can never occur today: a driver's check-in records stop times (and the last delivery departure sets `delivered_at`) but the load stays "in transit" until staff move it. If drivers update status in the app (P51's cousin), this needs its own decision: which statuses a driver may set (e.g. at pickup / loaded / at delivery / delivered) and whether check-in should move them. Larger item — not fixed.
