...
- Restrictive-policy ROLLOUT: `contractor_pay_setup` done 2026-09-18, migration `0011` (money-shaped pass), after the LIVE-UPDATING (realtime) batch of 19 (`0010`), SHARE LINKS + SMALL SETTINGS (`0009`) and the MONEY BATCH (`0007`) — **158 tables done, `user_roles` the ONLY company-bearing table still pending** (`company_members` permanently exempt on top of that); live totals **718 policies in `public`, 158 RESTRICTIVE**, linter 170. `user_roles` is deliberately last: every other policy in the database resolves through `has_role()` / `is_staff()`, which read it, so a restrictive predicate there changes the meaning of every other table's rules at once. The owner's live-update check PASSED again on 2026-09-18, AFTER the realtime batch (Driver Hub open in one window, a driver's dispatch status changed in another, hub updated without a refresh). Found earlier on the way: 8 tables are subscribed in source but NOT in the `supabase_realtime` publication, so those subscriptions deliver nothing today (pre-existing); the pre-check's realtime list wrongly included `loads` and `deductions`. (records 2026-09-17 2350 UTC — "the LIVE-UPDATING (realtime) batch"; 2026-09-18 1358 UTC — "contractor_pay_setup")
