...
Deno.serve(withErrorEnvelope(async (req) => {
  ...
  const { data: sheet, error: sheetError } = await supabase
    ...
    .select('id, operator_id, items:onboard_assignment_sheet_items(id, equipment_id)')
    .eq('id', body.sheetId)
    .single()
  if (sheetError || !sheet) {
  ...
  return ok({ success: true, sheetId: body.sheetId, releasedEquipment: equipmentIds.length })
}, 'delete-osas-sheet'))