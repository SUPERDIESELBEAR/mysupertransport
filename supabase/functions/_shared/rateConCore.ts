// Core of the rate-confirmation parser: gateway call, retry, unwrapping and
// normalization. Extracted from parse-rate-confirmation/index.ts so the inbound
// email ingest function can run the identical parse in-process under a
// service-role context — no HTTP hop, no token, and no public endpoint that a
// shared secret could turn into staff-equivalent access.
//
// parse-rate-confirmation/index.ts remains the staff-authenticated HTTP
// surface and owns auth only. Everything that decides WHAT a document parses
// to lives here, exactly once.
...
/** What the core returns: an HTTP-shaped outcome the caller maps to a response. */
...
/**
 * Runs one parse end to end: validation, the gateway call with bounded retry,
 * unwrapping, normalization and the empty-extraction guard. Auth is the
 * caller's business — the HTTP surface checks a staff role, the ingest
 * function runs under the service role after verifying the webhook signature.
 */
export async function parseRateConfirmationCore(
  body: ParseRequestBody,
  apiKey: string,
): Promise<ParseOutcome> {
  if (!body?.file_base64 || !body?.mime_type) {
    return { status: 400, body: { error: 'file_base64 and mime_type are required' } };
  }
  const isPdf = body.mime_type === 'application/pdf';
  const isImage = body.mime_type.startsWith('image/');
  if (!isPdf && !isImage) {
    return { status: 400, body: { error: 'Unsupported file type. Upload a PDF or an image.' } };
  }
  if (body.file_base64.length > 28_000_000) {
    return { status: 400, body: { error: 'That file is too large to parse. Use a file under 20 MB.' } };
  }
  ...
  if (!aiRes.ok) {
    ...
    if (aiRes.status === 429) return { status: 429, body: { error: 'AI is busy. Try again in a moment.' } };
    if (aiRes.status === 402) return { status: 402, body: { error: 'AI credits exhausted. Ask an admin to top up.' } };
    if (aiRes.status === 403) return { status: 403, body: { error: 'AI access is blocked for this workspace.' } };
    console.error('AI gateway failed', aiRes.status, t.slice(0, 400));
    if (aiRes.status >= 500) {
      return { status: 503, body: { error: 'The AI service is temporarily unavailable. Please try parsing again in a moment.' } };
    }
    return { status: 502, body: { error: `AI request failed (${aiRes.status})` } };
  }
  ...
  try { payload = JSON.parse(raw); } catch {
    ...
    return { status: 502, body: { error: 'AI returned invalid JSON' } };
  }
  ...
  const dateTime = (f: any) => {
    ...
    const rawConf: Conf = withTime ? s.confidence : (s.confidence === 'high' ? 'medium' : s.confidence);
    const confidence: Conf = rawConf === 'low' ? 'medium' : rawConf;
