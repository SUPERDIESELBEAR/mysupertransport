...
export interface MalfunctionNoticeData {
  ...
  /**
   * Read off the `eld_malfunction_events` row, never looked up. A notice filed
   * by a demo driver is watermarked forever, including on re-render after the
   * driver is cleared to live.
   */
  isDemo?: boolean;
}
...
export async function buildMalfunctionNotice(
  ...
  if (data.isDemo) drawDemoWatermark(page, bold, rgb, degrees);
