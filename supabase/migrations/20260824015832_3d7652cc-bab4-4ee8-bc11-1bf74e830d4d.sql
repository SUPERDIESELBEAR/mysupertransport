...
DECLARE
  ...
  c text;
BEGIN
  -- Policies that admit a role but no GRANT behind them. A policy decides which
  -- rows; a GRANT decides whether the table can be touched at all. Read from the
  -- catalog at call time: migration text can be right while the database is not,
  -- and information_schema.role_table_grants shows only grants the CALLING role
  -- is party to, so it reads empty for almost every table and proves nothing.
  FOR r IN
  ...
  LOOP
    ...
    FOREACH c IN ARRAY cmds LOOP
      -- authenticated: admitted explicitly or through PUBLIC (oid 0)
      ...
      -- anon: only when the predicate does not itself require a session
  ...
  -- Any policied table service_role cannot reach: edge functions and admin code
  -- run as service_role and bypass RLS, but not a missing grant.
  FOR r IN
  ...
  -- RLS left off on a table that has policies at all: the policies are inert.
  FOR r IN
    SELECT DISTINCT c.oid, c.relname
    FROM pg_policy p
    JOIN pg_class c ON c.oid = p.polrelid
...
COMMENT ON FUNCTION public.grant_parity_report() IS
  'Live catalog parity check: policies that admit a role the grants do not. Read pg_policy + has_table_privilege at call time; never information_schema.role_table_grants, which only exposes grants the calling role is party to.';
