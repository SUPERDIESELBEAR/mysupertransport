DO $$ BEGIN
  CREATE TYPE public.detention_clock_start AS ENUM ('appointment','arrival','gate_checkin');
EXCEPTION WHEN duplicate_object THEN NULL; END $$;

ALTER TABLE public.loads
  ADD COLUMN IF NOT EXISTS detention_free_time_minutes integer,
  ADD COLUMN IF NOT EXISTS detention_rate_per_hour numeric,
  ADD COLUMN IF NOT EXISTS detention_daily_cap numeric,
  ADD COLUMN IF NOT EXISTS detention_clock_start public.detention_clock_start,
  ADD COLUMN IF NOT EXISTS detention_notification_required boolean,
  ADD COLUMN IF NOT EXISTS detention_terms_note text;

COMMENT ON COLUMN public.loads.detention_free_time_minutes IS
  'Free time in MINUTES as stated on the rate confirmation. NULL means not stated — never default it.';
COMMENT ON COLUMN public.loads.detention_notification_required IS
  'Tri-state. TRUE required, FALSE explicitly not required, NULL not stated.';
...
BEGIN
  ...
  VALUES (
    ...
    NULLIF(p_load->>'detention_free_time_minutes','')::int,
    NULLIF(p_load->>'detention_rate_per_hour','')::numeric,
    NULLIF(p_load->>'detention_daily_cap','')::numeric,
    NULLIF(p_load->>'detention_clock_start','')::detention_clock_start,
    NULLIF(p_load->>'detention_notification_required','')::boolean,
    NULLIF(p_load->>'detention_terms_note',''),
