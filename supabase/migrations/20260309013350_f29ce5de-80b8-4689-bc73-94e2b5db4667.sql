...
-- Add the missing doc types used by OperatorDocumentUpload to the operator_doc_type enum
ALTER TYPE operator_doc_type ADD VALUE IF NOT EXISTS 'form_2290';
ALTER TYPE operator_doc_type ADD VALUE IF NOT EXISTS 'truck_title';
ALTER TYPE operator_doc_type ADD VALUE IF NOT EXISTS 'truck_photos';
ALTER TYPE operator_doc_type ADD VALUE IF NOT EXISTS 'truck_inspection';

-- Re-create the trigger function without the problematic enum cast
-- (TEXT comparison works fine since document_type stores the enum label as text)
