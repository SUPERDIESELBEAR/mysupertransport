ALTER TYPE public.load_document_type ADD VALUE IF NOT EXISTS 'reimbursement_proof';

ALTER TABLE public.pay_policies
  ADD COLUMN IF NOT EXISTS charge_pay_classes jsonb NOT NULL DEFAULT jsonb_build_object(
    'linehaul','revenue',
    'fsc','revenue',
    'detention','revenue',
    'stopoff','revenue',
...
COMMENT ON COLUMN public.pay_policies.charge_pay_classes IS
  'Charge classification key -> pay class. "revenue" splits at this policy''s percentage; "reimbursement" pays the actual cost back to whoever spent it. Lumper being a reimbursement is a policy choice, not a property of the charge type.';

ALTER TABLE public.load_charges
  ADD COLUMN IF NOT EXISTS funding_source text,
  ADD COLUMN IF NOT EXISTS actual_cost numeric,
  ADD COLUMN IF NOT EXISTS proof_document_id uuid REFERENCES public.load_documents(id) ON DELETE SET NULL;

ALTER TABLE public.load_charges
  DROP CONSTRAINT IF EXISTS load_charges_funding_source_check;
ALTER TABLE public.load_charges
  ADD CONSTRAINT load_charges_funding_source_check
  CHECK (funding_source IS NULL OR funding_source IN ('driver','company'));

ALTER TABLE public.load_charges
  DROP CONSTRAINT IF EXISTS load_charges_actual_cost_check;
ALTER TABLE public.load_charges
  ADD CONSTRAINT load_charges_actual_cost_check
  CHECK (actual_cost IS NULL OR actual_cost >= 0);

COMMENT ON COLUMN public.load_charges.funding_source IS
  'Who actually paid the cost: driver (reimbursed at cost on their settlement) or company (company revenue, absent from the driver settlement). Null until dispatch confirms.';
COMMENT ON COLUMN public.load_charges.actual_cost IS
  'What was actually spent, which may differ from what the broker bills. Null until proof is attached.';
COMMENT ON COLUMN public.load_charges.proof_document_id IS
  'Receipt or Comchek proof on this load. Where proof cannot be obtained, the existing document-exception path applies.';

CREATE INDEX IF NOT EXISTS load_charges_proof_document_id_idx
  ON public.load_charges (proof_document_id);
