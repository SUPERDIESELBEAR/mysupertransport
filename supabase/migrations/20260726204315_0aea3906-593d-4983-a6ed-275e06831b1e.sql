-- 1) Cascade trigger: set a scoped flag around the onboarding_status write
...
BEGIN
  IF NEW.direction = 'return' THEN
    PERFORM set_config('app.equipment_return_receipt', '1', true);
    UPDATE public.onboarding_status
       SET equipment_return_completed_at = now()
     WHERE operator_id = NEW.operator_id
       AND equipment_return_completed_at IS NULL;
    PERFORM set_config('app.equipment_return_receipt', '0', true);
  END IF;
  RETURN NEW;
END;
$function$;

-- 2) Guard: enforce_onboarding_status_self_update — allow the narrow return write
...
DECLARE
  ...
  v_new jsonb;
  v_key text;
  v_return_allowed text[] := ARRAY['equipment_return_completed_at','updated_at','updated_by'];
BEGIN
  ...
  IF current_setting('app.equipment_return_receipt', true) = '1' THEN
    v_old := to_jsonb(OLD);
    v_new := to_jsonb(NEW);
    FOR v_key IN SELECT jsonb_object_keys(v_new) LOOP
      IF v_new->v_key IS DISTINCT FROM v_old->v_key
         AND NOT (v_key = ANY(v_return_allowed)) THEN
        RAISE EXCEPTION 'Only the equipment return completion timestamp may be set by a return receipt upload';
      END IF;
    END LOOP;
    RETURN NEW;
  END IF;
...
-- 3) Guard: enforce_onboarding_status_operator_update
...
BEGIN
  ...
  IF current_setting('app.equipment_return_receipt', true) = '1' THEN
    v_allowed := v_allowed || ARRAY['equipment_return_completed_at'];
  END IF;
...
-- 4) Guard: enforce_onboarding_status_operator_column_whitelist
