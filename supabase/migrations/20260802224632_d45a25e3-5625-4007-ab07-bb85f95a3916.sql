CREATE OR REPLACE FUNCTION public.purge_rods_day(_day_id uuid, _reason text, _storage_owner text, _actor_id uuid DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- The three-argument form stays alive only until purge-rods-day is deployed
-- calling the four-argument one. It now delegates so the logic lives in one
-- place and the two cannot drift. Recorded in docs/deferred-removals.md.
...
AS $function$
BEGIN
  RETURN public.purge_rods_day(_day_id, _reason, _storage_owner, NULL::uuid);
END;
$function$;

REVOKE ALL ON FUNCTION public.purge_rods_day(uuid, text, text, uuid) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.purge_rods_day(uuid, text, text, uuid) TO service_role;