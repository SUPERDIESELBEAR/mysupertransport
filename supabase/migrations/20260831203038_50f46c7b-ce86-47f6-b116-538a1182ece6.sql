...
DECLARE
  ...
  v_doomed record;
  -- Charge diffing. A load save must not re-key a charge row: detention claims,
  -- proof documents and settlement line items hang off load_charges.id.
  v_cid uuid;
  v_ch_stop uuid;
  v_ch_type text;
  v_charge_ids uuid[] := '{}';
...
BEGIN
  ...
  -- Charges are DIFFED, never replaced. A surviving row keeps its id,
  -- created_at and created_by so foreign keys pointing at it stay valid.
  IF p_charges IS NOT NULL AND jsonb_typeof(p_charges) = 'array' THEN
    FOR v_charge IN SELECT * FROM jsonb_array_elements(p_charges) LOOP
      ...
      v_ch_stop := CASE
        ...
        ELSE NULL
      END;
      v_ch_type := COALESCE(NULLIF(v_charge->>'charge_type',''), 'other');

      v_cid := NULLIF(v_charge->>'id','')::uuid;

      -- Stop-attached charges are synthesised from the stop card and carry no
      -- id. They are identified by the stop they hang on plus their type.
      IF v_cid IS NULL AND v_ch_stop IS NOT NULL THEN
        SELECT c.id INTO v_cid
          FROM public.load_charges c
         WHERE c.load_id = p_load_id
           AND c.load_stop_id = v_ch_stop
           AND c.charge_type = v_ch_type
           AND NOT (c.id = ANY(v_charge_ids))
         ORDER BY c.created_at
         LIMIT 1;
      END IF;

      IF v_cid IS NOT NULL
         AND NOT (v_cid = ANY(v_charge_ids))
         AND EXISTS (SELECT 1 FROM public.load_charges c
                      WHERE c.id = v_cid AND c.load_id = p_load_id) THEN
        UPDATE public.load_charges SET
          load_stop_id = v_ch_stop,
          charge_type = v_ch_type,
          description = NULLIF(v_charge->>'description',''),
          amount = COALESCE(NULLIF(v_charge->>'amount','')::numeric, 0),
          source = COALESCE(NULLIF(v_charge->>'source',''), 'manual'),
          funding_source = NULLIF(v_charge->>'funding_source','')::text,
          actual_cost = NULLIF(v_charge->>'actual_cost','')::numeric,
          proof_document_id = NULLIF(v_charge->>'proof_document_id','')::uuid,
          updated_by = v_profile,
          updated_at = now()
        WHERE id = v_cid
          AND (
            load_stop_id IS DISTINCT FROM v_ch_stop
            OR charge_type IS DISTINCT FROM v_ch_type
            OR description IS DISTINCT FROM NULLIF(v_charge->>'description','')
            OR amount IS DISTINCT FROM COALESCE(NULLIF(v_charge->>'amount','')::numeric, 0)
            OR source IS DISTINCT FROM COALESCE(NULLIF(v_charge->>'source',''), 'manual')
            OR funding_source IS DISTINCT FROM NULLIF(v_charge->>'funding_source','')::text
            OR actual_cost IS DISTINCT FROM NULLIF(v_charge->>'actual_cost','')::numeric
            OR proof_document_id IS DISTINCT FROM NULLIF(v_charge->>'proof_document_id','')::uuid
          );
      ELSE
      ...
      v_charge_ids := array_append(v_charge_ids, v_cid);
    END LOOP;
  END IF;

  DELETE FROM public.load_charges
   WHERE load_id = p_load_id
     AND NOT (id = ANY(v_charge_ids));
