...
DECLARE
  ...
  c text;
BEGIN
  -- Only PERMISSIVE policies grant access. A RESTRICTIVE policy (the
  -- `tenant_isolation` cross-carrier rule) can only remove rows, so it never
  -- implies a grant; including it produced a false 'policy_without_grant'
  -- against parser_diagnostics, whose INSERT is deliberately RPC-only.
  FOR r IN
    ...
    WHERE p.polpermissive
      AND (coalesce(pg_get_expr(p.polqual, p.polrelid), '') !~* '^\s*false\s*$'
