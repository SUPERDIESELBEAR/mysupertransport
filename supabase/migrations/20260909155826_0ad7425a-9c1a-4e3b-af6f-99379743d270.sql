ALTER TABLE public.fuel_transactions ADD COLUMN IF NOT EXISTS merchant_name text;

COMMENT ON COLUMN public.fuel_transactions.merchant_name IS
  'Truck stop / merchant name from the provider export. NULL on every row imported before 2026-09-09 and on any provider that does not supply it. Added so cost per gallon can be reported BY TRUCK STOP, not only by city and state.';
...
BEGIN
  ...
  FOR r IN SELECT elem FROM jsonb_array_elements(COALESCE(_rows, '[]'::jsonb)) AS elem LOOP
    ...
    v_out := v_out || jsonb_build_object(
      ...
      'merchant_name', NULLIF(btrim(COALESCE(r->>'merchant_name','')), ''),
...
REVOKE ALL ON FUNCTION public.preview_fuel_import(jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.preview_fuel_import(jsonb) TO authenticated;

CREATE OR REPLACE FUNCTION public.commit_fuel_import(_file_name text, _provider text, _rows jsonb, _columns jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'extensions'
...
BEGIN
  ...
  FOR r IN SELECT elem FROM jsonb_array_elements(COALESCE(_rows, '[]'::jsonb)) AS elem LOOP
    ...
    INSERT INTO public.fuel_transactions (
      ...
      card_no, unit_no, driver_name, merchant_name, city, state, invoice_no, invoice_date, daycode,
      ...
      r->>'card_no', r->>'unit_no', r->>'driver_name',
      NULLIF(btrim(COALESCE(r->>'merchant_name','')), ''),
      r->>'city', r->>'state',
...
REVOKE ALL ON FUNCTION public.commit_fuel_import(text, text, jsonb, jsonb) FROM PUBLIC, anon;
