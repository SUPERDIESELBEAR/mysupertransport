-- Extend v_compliance_items to include per-driver Registration and Form 2290 documents.
...
fleet AS (
  SELECT 'fleet'::text AS entity_kind,
    ...
    LEFT JOIN public.inspection_documents d
      ON d.scope = 'company_wide'::public.inspection_doc_scope AND d.name = doc.name
),
drivers AS (
  SELECT 'driver'::text AS entity_kind,
    ...
    JOIN public.onboarding_status os ON os.operator_id = o.id
    CROSS JOIN (VALUES ('CDL'::text,'CDL (Front)'::text),
                       ('Medical Certificate'::text,'Medical Certificate'::text),
                       ('IRP Registration (cab card)'::text,'IRP Registration (cab card)'::text),
                       ('Registration'::text,'Registration'::text),
                       ('Form 2290'::text,'Form 2290'::text)) doc(doc_key, name)
    LEFT JOIN public.inspection_documents d
      ON d.scope = 'per_driver'::public.inspection_doc_scope
