-- The display copy is a second object the row owns (added with the HEIC path).
-- Without it here, purging a day left the converted JPEG stranded in storage.
...
BEGIN
  ...
  END IF;
  -- The re-encoded display sibling. Guarded against the degenerate case where
  -- it equals the original, so the caller is not asked to delete it twice.
