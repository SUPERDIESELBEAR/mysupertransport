ALTER TABLE public.rods_days
  ADD COLUMN IF NOT EXISTS certification_signature_validation jsonb;

-- New signature: adds p_signature_validation, defaulted so that nothing about
-- the existing call shape changes. The seven-argument form below is KEPT until
-- no queued client entry can still carry it (docs/deferred-removals.md).
...
REVOKE ALL ON FUNCTION public.certify_rods_day(uuid,text,text,text,text,uuid,jsonb,jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.certify_rods_day(uuid,text,text,text,text,uuid,jsonb,jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.certify_rods_day(uuid,text,text,text,text,uuid,jsonb,jsonb) TO service_role;

-- The seven-argument form stays for in-flight queued calls from clients that
-- predate this deploy. It now delegates, so the certification logic exists in
-- exactly one place and cannot drift; a caller that never computed a
-- validation simply records none.
CREATE OR REPLACE FUNCTION public.certify_rods_day(
  _day_id uuid,
  _legal_name text,
  _signature_path text,
...
SET search_path TO 'public', 'extensions'
AS $function$
  SELECT public.certify_rods_day(
    _day_id, _legal_name, _signature_path, _pdf_path, _device_info,
    p_certification_token, p_changes, NULL::jsonb
  );
$function$;

REVOKE ALL ON FUNCTION public.certify_rods_day(uuid,text,text,text,text,uuid,jsonb) FROM PUBLIC, anon;
