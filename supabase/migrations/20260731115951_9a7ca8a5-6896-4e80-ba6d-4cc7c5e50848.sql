-- The previous caller check read current_user, which inside a SECURITY DEFINER
-- function is always the function OWNER, never the caller. The check could
-- therefore never pass -- the same class of never-executed defect this audit
-- has been finding. Identity of the caller must come from the request role
-- claim (PostgREST) or the session role (direct admin connection).
