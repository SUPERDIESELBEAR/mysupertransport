  ...
  IF current_setting('app.equipment_return_receipt', true) = '1' THEN
    ...
    FOR v_key IN SELECT jsonb_object_keys(v_new) LOOP
      IF v_new->v_key IS DISTINCT FROM v_old->v_key
         AND NOT (v_key = ANY(v_return_allowed))
         AND NOT (v_key = ANY(v_generated)) THEN
        v_bad := v_bad || v_key;
      END IF;
    END LOOP;
    IF array_length(v_bad, 1) > 0 THEN
      RAISE EXCEPTION 'Return receipt update touched unexpected fields: %', array_to_string(v_bad, ', ');
    END IF;
    RETURN NEW;
  END IF;
