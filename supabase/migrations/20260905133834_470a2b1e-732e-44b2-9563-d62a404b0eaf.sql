-- Module 6, Pass 2 — columns found by name, and the column set remembered.

ALTER TYPE public.fuel_line_type ADD VALUE IF NOT EXISTS 'diesel1';
ALTER TYPE public.fuel_line_type ADD VALUE IF NOT EXISTS 'unleaded';
ALTER TYPE public.fuel_line_type ADD VALUE IF NOT EXISTS 'cng';
ALTER TYPE public.fuel_line_type ADD VALUE IF NOT EXISTS 'lng';
ALTER TYPE public.fuel_line_type ADD VALUE IF NOT EXISTS 'lpg';
ALTER TYPE public.fuel_line_type ADD VALUE IF NOT EXISTS 'reefer_cng';
ALTER TYPE public.fuel_line_type ADD VALUE IF NOT EXISTS 'reefer_lng';
ALTER TYPE public.fuel_line_type ADD VALUE IF NOT EXISTS 'reefer_lpg';
ALTER TYPE public.fuel_line_type ADD VALUE IF NOT EXISTS 'oil';
ALTER TYPE public.fuel_line_type ADD VALUE IF NOT EXISTS 'tax';

ALTER TABLE public.fuel_import_batches
  ADD COLUMN IF NOT EXISTS recognized_columns    text[] NOT NULL DEFAULT '{}',
  ADD COLUMN IF NOT EXISTS unrecognized_columns  text[] NOT NULL DEFAULT '{}',
  ADD COLUMN IF NOT EXISTS missing_optional_columns text[] NOT NULL DEFAULT '{}';

COMMENT ON COLUMN public.fuel_import_batches.recognized_columns IS
  'Known column names present in the imported file. Compared against on the next import so a forgotten report checkbox is reported before commit.';

-- The writer is unchanged in every respect that matters (dedupe key, card
-- resolution, match semantics); it gains one argument carrying the column set
-- so the batch can remember what the file looked like. The three-argument form
-- is dropped so there is still exactly ONE writer, not two.
DROP FUNCTION IF EXISTS public.commit_fuel_import(text, text, jsonb);

CREATE OR REPLACE FUNCTION public.commit_fuel_import(
  _file_name text, _provider text, _rows jsonb, _columns jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO public.fuel_import_batches (
    provider, file_name, imported_by, row_count,
    recognized_columns, unrecognized_columns, missing_optional_columns)
  VALUES (COALESCE(NULLIF(_provider, ''), 'multiservice')::public.fuel_provider,
          ...
          COALESCE(ARRAY(SELECT jsonb_array_elements_text(COALESCE(_columns->'recognized', '[]'::jsonb))), '{}'),
          COALESCE(ARRAY(SELECT jsonb_array_elements_text(COALESCE(_columns->'unrecognized', '[]'::jsonb))), '{}'),
          COALESCE(ARRAY(SELECT jsonb_array_elements_text(COALESCE(_columns->'missing_optional', '[]'::jsonb))), '{}'))
...
REVOKE ALL ON FUNCTION public.commit_fuel_import(text, text, jsonb, jsonb) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.commit_fuel_import(text, text, jsonb, jsonb) FROM anon;
GRANT EXECUTE ON FUNCTION public.commit_fuel_import(text, text, jsonb, jsonb) TO authenticated;
GRANT EXECUTE ON FUNCTION public.commit_fuel_import(text, text, jsonb, jsonb) TO service_role;