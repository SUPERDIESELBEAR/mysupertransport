...
BEGIN
  ...
  -- Internal cascade bypass: the sync_ica_completion_to_onboarding trigger
  -- sets this transaction-local flag when propagating an ICA signature.
  IF current_setting('app.ica_sync_cascade', true) = '1' THEN
    RETURN NEW;
  END IF;

  -- (Original guard body preserved below)
