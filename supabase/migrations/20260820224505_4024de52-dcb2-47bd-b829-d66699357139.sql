-- Load editing: change history table + atomic update function.

CREATE TABLE IF NOT EXISTS public.load_change_history (
  ...
  reason text,
  change_source text NOT NULL DEFAULT 'manual_ui',
  changed_by uuid REFERENCES public.profiles(id) ON DELETE SET NULL,
  changed_at timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX IF NOT EXISTS load_change_history_load_id_idx
  ON public.load_change_history (load_id, changed_at DESC);

GRANT SELECT ON public.load_change_history TO authenticated;
GRANT ALL ON public.load_change_history TO service_role;

ALTER TABLE public.load_change_history ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS load_change_history_staff_read ON public.load_change_history;
CREATE POLICY load_change_history_staff_read
  ON public.load_change_history FOR SELECT TO authenticated
  USING (
...
CREATE OR REPLACE FUNCTION public.update_load_with_stops(
  p_load_id uuid,
  p_load jsonb,
  p_stops jsonb,
  p_charges jsonb DEFAULT '[]'::jsonb,
  p_reason text DEFAULT NULL::text,
  p_financial_unlock_reason text DEFAULT NULL::text,
  p_ack_stop_data_loss boolean DEFAULT false
)
RETURNS uuid
LANGUAGE plpgsql
SECURITY DEFINER
