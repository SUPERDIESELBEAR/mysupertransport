-- ============================================================================
-- Demo carrier, stage 3, pass 3d: the per-carrier apply link and the letterhead.
--
-- 1. anon's unused INSERT privilege on applications is revoked. The public form
--    has only ever written through save_application_draft /
--    submit_application_draft, both SECURITY DEFINER. Pass 3b reported anon held
--    nothing; pg_class.relacl showed 'anon=a/postgres'. The grant was real and
--    unused; the 42501 3b observed came from RLS, not from the grant.
-- 2. carrier_profile gains a public apply slug and the applicant-facing locality
--    that until now lived as a hard-coded constant in the front end.
-- 3. Two anon-callable definer readers return ONLY public identity fields.
-- 4. save_application_draft resolves the carrier from the SLUG, server-side.
-- ============================================================================

REVOKE INSERT ON public.applications FROM anon;

ALTER TABLE public.carrier_profile
  ADD COLUMN IF NOT EXISTS apply_slug text,
  ADD COLUMN IF NOT EXISTS applicant_locality text;

COMMENT ON COLUMN public.carrier_profile.apply_slug IS
  'Short public slug for this carrier''s driver-application link: /apply/<slug>. Unique, lowercase.';
COMMENT ON COLUMN public.carrier_profile.applicant_locality IS
  'City and state shown on applicant-facing disclosures. Deliberately NOT the street address.';

UPDATE public.carrier_profile
   SET apply_slug         = 'supertransport',
       applicant_locality = 'Pleasant Hill, Missouri'
 WHERE usdot_number = '2309365';

ALTER TABLE public.carrier_profile
  ADD CONSTRAINT carrier_profile_apply_slug_format
  CHECK (apply_slug IS NULL OR apply_slug ~ '^[a-z0-9](?:[a-z0-9-]{1,30}[a-z0-9])$');

CREATE UNIQUE INDEX IF NOT EXISTS carrier_profile_apply_slug_unique
  ON public.carrier_profile (lower(apply_slug))
  WHERE apply_slug IS NOT NULL;

-- ---------------------------------------------------------------------------
-- The public identity reader.
--
-- Returns FIVE fields and nothing else: legal name, applicant locality, USDOT,
-- MC, and the slug itself. No id, no street address, no terminal, no ingest
-- address, no timestamps. The applicant's disclosures need exactly these.
--
-- p_slug NULL keeps the bare /apply route working while exactly one carrier
-- exists. With two or more it returns NO ROW -- it never guesses.
-- ---------------------------------------------------------------------------
CREATE OR REPLACE FUNCTION public.carrier_public_identity(p_slug text DEFAULT NULL)
RETURNS TABLE(legal_name text, applicant_locality text, usdot_number text, mc_number text, apply_slug text)
LANGUAGE sql
STABLE
SECURITY DEFINER
SET search_path TO 'public'
AS $$
  SELECT c.legal_name, c.applicant_locality, c.usdot_number, c.mc_number, c.apply_slug
  FROM public.carrier_profile c
  WHERE CASE
          WHEN nullif(btrim(coalesce(p_slug, '')), '') IS NOT NULL
            THEN lower(c.apply_slug) = lower(btrim(p_slug))
          ELSE (SELECT count(*) FROM public.carrier_profile) = 1
        END
  LIMIT 1;
$$;

REVOKE ALL ON FUNCTION public.carrier_public_identity(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.carrier_public_identity(text) TO anon, authenticated, service_role;

-- A returning applicant holds a draft token, not a slug. Same five fields, read
-- from the carrier the draft already belongs to, so a resumed application lands
-- on its own carrier's form.
CREATE OR REPLACE FUNCTION public.carrier_identity_for_draft(p_draft_token text)
RETURNS TABLE(legal_name text, applicant_locality text, usdot_number text, mc_number text, apply_slug text)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  ...
  JOIN public.carrier_profile c ON c.id = a.company_id
  WHERE a.draft_token = nullif(btrim(coalesce(p_draft_token, '')), '')
  LIMIT 1;
$$;

REVOKE ALL ON FUNCTION public.carrier_identity_for_draft(text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.carrier_identity_for_draft(text) TO anon, authenticated, service_role;

-- ---------------------------------------------------------------------------
-- save_application_draft: the carrier comes from the SLUG.
--
-- The browser sends payload.carrier_slug -- never a carrier id. The id is
-- resolved here, inside a definer function, from carrier_profile. An unknown
-- slug RAISES 'unknown_carrier' and nothing is written.
--
-- Slug omitted (the bare /apply route) leaves company_id NULL on the INSERT, so
-- the 3b stamp trigger applies its sole-carrier fallback -- and still refuses
-- once a second carrier exists.
--
-- An existing draft keeps the carrier it was filed under: the carrier is set on
-- INSERT only, never moved by a later save.
-- ---------------------------------------------------------------------------
